...
asn1c_make_identifier(enum ami_flags_e flags, asn1p_expr_t *expr, ...) {
    ...
    int need_underscore_at_the_end = 0;
    /*
      ...
      //in asn1c it is made via toupper() of the first letter.
      //But C++ class names starts with upper-case letter, therefore we can't 
      //use upper-case letter for the identifier (class field).
			need_underscore_at_the_end = 1;
			/* Fall through */
		}
...
char* asn1c_make_namespace_name(arg_t* arg, asn1p_expr_t *expr, int terminal) {
  asn1p_expr_t temp;
  memset(&temp, 0, sizeof(temp));
  if (terminal)
    expr = asn1f_find_terminal_type_ex(arg->asn, expr);
  if (expr)
    temp.Identifier = expr->module->ModuleName;
  temp.spec_index = -1;
  return asn1c_make_identifier(AMI_CHECK_RESERVED, &temp, (char*)0);
}
...
//		}
		...
		|| asn1c_type_fits_long(arg, expr)) {
			...
			case TNF_RSAFE:
				if(expr->expr_type == ASN_BASIC_REAL)
					...
					return "UnsignedNativeInteger";
				else if (expr->expr_type == ASN_BASIC_ENUMERATED)
    ...
    if (expr->expr_type == ASN_BASIC_NULL) {
      return asn1c_make_identifier(0, exprid, 
              exprid?"t":typename, exprid?(char*)0:"t", (char*)0);
    } else {
      if (expr->expr_type & ASN_CONSTR_MASK)
        return asn1c_make_identifier(0, exprid, "class ", exprid?(char*)0:typename, (char*)0);
      else
        return asn1c_make_identifier(0, exprid, exprid?(char*)0:typename, (char*)0);
    }
