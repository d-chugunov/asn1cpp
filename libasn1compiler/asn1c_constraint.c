...
asn1c_emit_constraint_checking_code(arg_t *arg) {
  ...
  char *tname = make_type_for_constraint(arg, expr); 
	if(produce_st) {
    if (ASN_BASIC_BOOLEAN == etype || 
            ASN_BASIC_NULL == etype || 
            ((ASN_BASIC_INTEGER == etype || 
                    ASN_BASIC_ENUMERATED == etype || 
                    ASN_BASIC_REAL) 
              && (arg->flags & A1C_USE_WIDE_TYPES))) 
    {
      OUT("const %s *st = static_cast<const %s *>(sptr);\n", tname, tname);
    } else {
      if (ASN_CONSTR_SEQUENCE_OF == etype || ASN_CONSTR_SET_OF == etype) {
        tname = "constr_SET_OF";
      } 
      OUT("const %s *st = static_cast<const %s*>(static_cast<const AsnAbstractType*>(sptr));\n", tname, tname);
    }
	}
	...
	OUT(") {\n");
      ...
      if (AMT_TYPEREF == expr->meta_type) {
        OUT("%s_%d_inherit_TYPE_descriptor(td);\n", 
                   asn1c_make_identifier(0, expr, (char*)0), expr->_type_unique_index);
      }
			OUT("return %s_constraint(td, sptr, ctfailcb, app_key);\n", 
              etype == ASN_CONSTR_SEQUENCE_OF ? "SEQUENCE_OF" : "SET_OF");
			break;
		default:
			OUT("/* Constraint check succeeded */\n");
			OUT("return 0;\n");
...
asn1c_emit_constraint_tables(arg_t *arg, int got_size) {
	...
	OUT("static int check_permitted_alphabet_%d(const %s* st) {\n", 
          arg->expr->_type_unique_index, make_type_for_constraint(arg, arg->expr));
	INDENT(+1);
	if(utf8_full_alphabet_check) {
		OUT("if(st->GetLength() < 0)\n");
		OUT("\treturn -1; /* Alphabet (sic!) test failed. */\n");
		OUT("\n");
	} 
  //asn1c has this else branch but it results that code doesn't check 
  //alphabetic constraint. Therefore I consider this as a bug and have fixed it
  //else {
		if(use_table) {
...
emit_alphabet_check_loop(arg_t *arg, asn1cnst_range_t *range) {
	...
	case ASN_STRING_UTF8String:
		OUT("const uint8_t *ch = reinterpret_cast<const uint8_t*>(st->data());\n");
	...
	if(range) {
		OUT("if(!(");
		int ret = emit_range_comparison_code(arg, range, "cv", 0, natural_stop);
		OUT(")) return -1;\n");
    if (!ret)
      OUT("(void)cv;\n");
	} else {
...
emit_size_determination_code(arg_t *arg, asn1p_expr_type_e etype) {
	...
	case ASN_BASIC_BIT_STRING:
		OUT("if(st->size() > 0) {\n");
		OUT("\t/* Size in bits */\n");
		OUT("\tsize = 8 * st->size() - (st->GetBitsUnused() & 0x07);\n");
		OUT("} else {\n");
		OUT("\tsize = 0;\n");
		OUT("}\n");
		break;
...
emit_value_determination_code(arg_t *arg, asn1p_expr_type_e etype, asn1cnst_range_t *r_value) {
	...
	case ASN_BASIC_ENUMERATED:
		if(asn1c_type_fits_long(arg, arg->expr) == FL_FITS_UNSIGN) {
			OUT("value = (unsigned long)*static_cast<const UnsignedNativeInteger *>(static_cast<const AsnAbstractType*>(sptr));\n");
		} else if(asn1c_type_fits_long(arg, arg->expr) != FL_NOTFIT) {
			OUT("value = (long)*static_cast<const ");
      if (etype == ASN_BASIC_ENUMERATED)
        OUT("NativeEnumerated");
      else
        OUT("NativeInteger");
      OUT(" *>(static_cast<const AsnAbstractType*>(sptr));\n");
		} else {
			/*
	...
	case ASN_BASIC_REAL:
		if(arg->flags & A1C_USE_WIDE_TYPES) {
			...
			OUT("value = (double)*static_cast<const NativeReal *>(static_cast<const AsnAbstractType*>(sptr));\n");
		}
		break;
	case ASN_BASIC_BOOLEAN:
		OUT("value = ((_BOOLEANType)*static_cast<const BOOLEAN *>(static_cast<const AsnAbstractType*>(sptr))) ? 1 : 0;\n");
		break;
	default:
