...
#define	INDENT(val)	do {                                    \
    INDENT_LEVEL += (val);                                  \
    if (INDENT_LEVEL < 0) {                                 \
      fprintf(stderr, "INDENT_LEVEL = %d\n", INDENT_LEVEL); \
      INDENT_LEVEL = 0;                                     \
      FATAL("INDENT_LEVEL is less than 0", INDENT_LEVEL);   \
    }                                                       \
  } while (0)
...
#define	EMBED(ev)	do {					\
      ...
      _tmp.gen_inner_class_declaration = 0;     \
			_tmp.expr = ev;				\
...
#define	EMBED_CLASS_DECLARATION(ev)	do {					\
		int saved_target = arg->target->target;		\
		REDIR(OT_TYPE_DECLS);				\
		arg->embed++;					\
		INDENTED(arg_t _tmp = *arg;			\
      _tmp.gen_inner_class_declaration = 1;     \
			_tmp.expr = ev;				\
			_tmp.default_cb(&_tmp);			\
		);						\
		arg->embed--;					\
