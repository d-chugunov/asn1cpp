...
asn1f_class_access(arg_t *arg, asn1p_module_t *mod, asn1p_expr_t *rhs_pspecs, asn1p_ref_t *ref) {
    ...
    //the asn1c code is commented and replaced with another code because there is
    //suspicion on error. If we reference to ABSTRACT-SYNTAX.&Type from 
    //different modules, then the result code will contain type ANY with 
    //namespace of one of modules. It causes compile error.
...
//		}
		if(TQ_FIRST(&classfield->members)) {
			/* Already have something */
      return TQ_FIRST(&classfield->members);
		} else {
			expr = asn1p_expr_new(classfield->_lineno, mod);
			expr->expr_type = ASN_TYPE_ANY;
			expr->meta_type = AMT_TYPE;
