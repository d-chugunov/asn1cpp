...
asn_TYPE_descriptor_t asn_DEF_REAL = {
	...
	ber_decode_primitive<REAL>,
	der_encode_primitive<REAL>,
...
REAL__dump(double d, int canonical, asn_app_consume_bytes_f *cb, void *app_key) {
	...
	if(isnan(d)) {
		buf = const_cast<char*>(specialRealValue[SRV__NOT_A_NUMBER].string);
		buflen = specialRealValue[SRV__NOT_A_NUMBER].length;
		return (cb(buf, buflen, app_key) < 0) ? -1 : buflen;
	} else if(!_asn_isfinite(d)) {
		if(copysign(1.0, d) < 0.0) {
			buf = const_cast<char*>(specialRealValue[SRV__MINUS_INFINITY].string);
			buflen = specialRealValue[SRV__MINUS_INFINITY].length;
		} else {
			buf = const_cast<char*>(specialRealValue[SRV__PLUS_INFINITY].string);
...
static int asn_REAL2double(const std::vector<uint8_t>* st, double* d) {
  if (!st || !d)
    return -1;
  else 
    return REAL::ToDouble(*st, *d) ? 0 : -1;
}
...
REAL_print(asn_TYPE_descriptor_t *td, const void *sptr, int ilevel,
  ...
  if (sptr) {
    ...
    st = static_cast<const REAL*>(structPtr);
  }
	ssize_t ret;
	double d;
...
bool REAL::ToDouble(const uint8_t* buf, size_t size, double& d) {
 ...
 */
bool REAL::FromDouble(double dbl_value) {
      ...
      st.resize(2);
		...
		if(isnan(dbl_value)) {
			...
			if(copysign(1.0, dbl_value) < 0.0) {
				st[0] = 0x41;	/* MINUS-INFINITY */
			} else {
				st[0] = 0x40;	/* PLUS-INFINITY */
			}
			st[1] = 0;
			st.resize(1);
		} else {
