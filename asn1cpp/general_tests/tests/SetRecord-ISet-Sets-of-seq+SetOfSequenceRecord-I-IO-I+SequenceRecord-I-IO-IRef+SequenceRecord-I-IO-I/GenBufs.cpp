...
std::vector<uint8_t> Buf1() {
  ...
  SetRecord_ISet_Sets_of_seq_t rec;
  memset(&rec, 0, sizeof(rec));
  asn_set_add(&rec.set.field1, &val1);
  asn_set_add(&rec.set.field3, &val3);
  asn_enc_rval_t rval;
  rval = der_encode_to_buffer(&asn_DEF_SetRecord_ISet_Sets_of_seq, &rec, (void*)buf.data(), buf.size());
  if (rval.encoded >= 0) {
...
std::vector<uint8_t> Buf2() {
  ...
  memset(&rec, 0, sizeof(rec));
  asn_set_add(&rec.set.field1, &val1);
  rec.set.field2 = (struct SetOfSequenceRecord_I_IO_I*)malloc(sizeof(struct SetOfSequenceRecord_I_IO_I));
  memset(rec.set.field2, 0, sizeof(*rec.set.field2));
  asn_set_add(rec.set.field2, &val2);
  asn_set_add(&rec.set.field3, &val3);
  rec.set.field4 = (struct SetRecord_ISet_Sets_of_seq::set::field4*)malloc(sizeof(*rec.set.field4));
