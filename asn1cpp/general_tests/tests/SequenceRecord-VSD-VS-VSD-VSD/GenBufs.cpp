...
std::vector<uint8_t> Buf1() {
  ...
  uint8_t val3[] = "666";
  uint8_t val4[] = "222";
  SequenceRecord_VSD_VS_VSD_VSD_t rec;
  memset(&rec, 0, sizeof(rec));
  rec.val2.buf = val2;
  rec.val2.size = sizeof(val2) - 1;
  asn_enc_rval_t rval;
  rval = der_encode_to_buffer(&asn_DEF_SequenceRecord_VSD_VS_VSD_VSD, &rec, (void*)buf.data(), buf.size());
  if (rval.encoded >= 0) {
...
std::vector<uint8_t> Buf2() {
  ...
  SequenceRecord_VSD_VS_VSD_VSD_t rec;
  memset(&rec, 0, sizeof(rec));
  rec.val1 = (VisibleString_t*)malloc(sizeof(*rec.val1));
  rec.val1->buf = val1;
  rec.val1->size = sizeof(val1) - 1;
  rec.val2.buf = val2;
  rec.val2.size = sizeof(val2) - 1;
  rec.val3 = (VisibleString_t*)malloc(sizeof(*rec.val3));
  rec.val3->buf = val3;
  rec.val3->size = sizeof(val3) - 1;
  rec.val4 = (VisibleString_t*)malloc(sizeof(*rec.val4));
  rec.val4->buf = val4;
