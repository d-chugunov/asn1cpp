...
std::vector<uint8_t> Buf1() {
  ...
  SetRecord_ThirdExtension_t rec;
  memset(&rec, 0, sizeof(rec));
  rec.a = 123;
  rec.z = 567.123;
  asn_enc_rval_t rval;
  rval = der_encode_to_buffer(&asn_DEF_SetRecord_ThirdExtension, &rec, (void*)buf.data(), buf.size());
  if (rval.encoded >= 0) {
...
std::vector<uint8_t> Buf3() {
  ...
  rec.c = (long*)malloc(sizeof(*rec.c));
  *rec.c = 546;
  rec.d = (struct SetRecord_ThirdExtension_t::d*)malloc(sizeof(*rec.d));
