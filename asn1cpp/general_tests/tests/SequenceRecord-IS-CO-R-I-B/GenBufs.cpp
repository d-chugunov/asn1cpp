#include "GenBufs.hpp"
#include "SequenceRecord-IS-CO-R-I-B.h"

const size_t DEFAULT_BUF_SIZE = 1024;

std::vector<uint8_t> Buf1() {
  std::vector<uint8_t> buf(DEFAULT_BUF_SIZE);
  SequenceRecord_IS_CO_R_I_B_t rec;
  memset(&rec, 0, sizeof(rec));
  rec.set.boolean = 1;
  rec.set.choice = (struct SequenceRecord_IS_CO_R_I_B_t::set::choice*)malloc(sizeof(*rec.set.choice));
  ...
  asn_enc_rval_t rval;
  rval = der_encode_to_buffer(&asn_DEF_SequenceRecord_IS_CO_R_I_B, &rec, (void*)buf.data(), buf.size());
  if (rval.encoded >= 0) {
